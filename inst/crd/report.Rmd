...
####################################finish
  ...
  ########################### TEXT INFERENCE
   if(input$submit > 0){
      ...
      else {cat("Treatment means are not significantly different")
      }
   }
  #########################
  tags$br()
 ...
 if(input$submit > 0){
      if(input$filerepli=='equal'){
        ...
        if(result[1,5]>0.05){return()}
          ...
          result<-out$duncan
           kable(result,caption = "DMRT",digits=3,align = 'c',row.names = FALSE)  
        }
          ...
          result<-out$statistics
           kable(result,caption = "Tukey",digits=3,align = 'c')  
        } 
      }
   else {return()}
